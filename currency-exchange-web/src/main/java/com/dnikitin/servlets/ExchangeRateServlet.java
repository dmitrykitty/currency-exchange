...
import com.dnikitin.exceptions.InvalidCurrencyPairException;
...
public class ExchangeRateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (pathInfo == null || pathInfo.equals("/")) {
            throw new InvalidCurrencyPairException("Missing currency pair");
        }

        pathInfo = pathInfo.substring(1); //remove /
        if(!pathInfo.matches("[A-Z]{6}")){
            throw new InvalidCurrencyPairException("Invalid currency pair. Correct format <code1><code2>, for example USDEUR");
        }
